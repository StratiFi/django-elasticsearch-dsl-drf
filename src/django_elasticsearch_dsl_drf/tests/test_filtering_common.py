...
class TestFilteringCommon(BaseRestFrameworkTestCase, AddressesMixin, BooksMixin):
    ...
    def _field_filter_value(self, field_name, value, count):
        ...
        url = self.base_url[:]
        data = {}
        response = self.client.get(url + f"?{field_name}={value}", data)
    ...
    def _field_filter_multiple_values(self, url, field_name, values, count):
        ...
        # url = self.base_url[:]
        data = {}
        params = "&".join([f"{field_name}={__v}" for __v in values])
        response = self.client.get(url + f"?{params}", data)
    ...
    def _field_filter_term(self, field_name, filter_value):
        ...
        filtered_response = self.client.get(url + f"?{field_name}={filter_value}", data)
    ...
    def test_field_filter_regexp(self):
        ...
        return self._field_filter_value(
            "title__regexp",
            f"{self.prefix[0:2]}.{{8}}{self.prefix[10:13]}.*",
            self.prefix_count,
        )
    ...
    def _field_filter_terms_list(self, field_name, in_values, count):
        ...
        url = self.base_url[:]
        data = {}
        url_parts = [f"{field_name}={val}" for val in in_values]
    ...
    def test_field_filter_wildcard(self):
        ...
        """
        return self._field_filter_value(
            "title__wildcard", f"*{self.prefix[1:6]}*", self.prefix_count
        )
